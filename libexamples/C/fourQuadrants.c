 ...
 * Generate only 10 layer of elements in a 40 x 20 rectangle.
 *
...
int main ( int argc, char* argv[] ) {
  ...
  /** Step 6: Assign custom solutions to the elements */
 
  MMG3D_Set_solSize(mmgMesh,mmgSol,MMG5_Vertex,mmgMesh->np,MMG5_Scalar);
  part = (int *)malloc(mmgMesh->ne*sizeof(int));
  ...
  for( int ip = 1; ip <= mmgMesh->np; ip++ ) {
    ppt = &mmgMesh->point[ip];
    if( (ppt->c[0] == xlim) || (ppt->c[1] == ylim) )
      mmgSol->m[ip] = 0.1;
    else
      mmgSol->m[ip] = 1.0;
  }
